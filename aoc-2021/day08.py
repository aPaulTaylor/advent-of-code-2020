...
def process_digits(digits):
    digits_sorted = [''.join(sorted(x)) for x in sorted(digits,key=len)]
    ...
    for d in digits_sorted:
        if d not in digits_dict:
            digit_sig=tuple((num_common_segs(d,x) for x in [digits_sorted[i] for i in [0,2,1,9]]))
...
for input_line in input:
    ...
    output_sum += sum(decoded_digits[i]*(10**(3-i)) for i in range(4))
print(output_sum)